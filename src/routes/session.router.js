...
router.get('/current', async (req, res) => {
    try {
        ...
        req.logger.error("Error al obtener el usuario y el carrito:", error);
...
router.get("/githubcallback", passport.authenticate("github", { failureRedirect: "/login" }), async (req, res) => {
    try {
        ...
        req.logger.error("Error in GitHub authentication callback:", error);
...
router.post("/login", async (req, res, next) => {
    passport.authenticate("login", { failureRedirect: "/api/sessions/faillogin" }, async (err, user, info) => {
        ...
        try {
            ...
            await UserModel.findOneAndUpdate({ email: user.email }, { last_connection: new Date() });
            req.session.login = true;
            res.redirect("/profile");
        } catch (error) {
...
router.get("/faillogin", async (req, res ) => {
    req.logger.log("Fallo la estrategia")
